...
-- =============================================
ALTER PROCEDURE sp_camp_ProcesarGestionCalling
	@Periodo int, @IdNomina int
...
BEGIN
	...
	DECLARE @Fecha_movimiento int

	DECLARE @isOpen int = 0 , @isDes int = 0 , @isReb int = 0

	DECLARE cCALL CURSOR FOR
	SELECT REPLACE(glosa,'.','/') glosa, Rut_OK, DV_OK, Fecha_movimiento
	FROM BD_ODS..TabContactos_CallOut_Atento
	WHERE periodo = @Periodo

	OPEN cCALL  
	FETCH NEXT FROM cCALL INTO @String, @rut, @dv, @Fecha_movimiento
	...
	BEGIN  
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						INSERT INTO BD_ODS.dbo.TabMae_AfiliadosFallecidos VALUES (@rut,@dv,@Fecha_movimiento)
					END
			...
			-------------------------------------------
			---INSERT FINAL CON TODAS LAS VARIABLES	---
			INSERT INTO BD_CAMPANIAS..TabCmp_GestionCamp values (@IdNomina,@rut,@Fecha_movimiento,@isOpen,@isDes,@isReb,0,null,null)
